#! /bin/bash
#
# Filename: k8s_server_status.bash, part of advlabtools; https://github.com/ynakaoku/advlabtools
...
echo 'Check server status on instances...'
for instance in "${servers[@]}" ; do
    echo "${instance}" status:
    if [ ${stypes[i]} = "kubernetes" ] ; then
        $SCRIPT_DIR/optlib/k8s_server_status_opt.bash ${instance}
    else
        ssh root@"${instance}" 'bash -s' < $SCRIPT_DIR/optlib/server_status_opt.bash
    fi
done
echo 'Finished.'
